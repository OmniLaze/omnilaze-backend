...
let OrdersService = class OrdersService {
    ...
    async submitOrder(orderId, userId) {
        ...
        if (existingOrder.userId !== userId)
            return { success: false, message: '您无权操作此订单' };
    ...
    async updateOrderFeedback(orderId, userId, rating, feedback) {
        if (!orderId)
            return { success: false, message: '订单ID不能为空' };
        if (!userId)
            return { success: false, message: '用户ID不能为空' };
        ...
        if (exists.userId !== userId)
            return { success: false, message: '您无权评价此订单' };
    ...
    async importArrivalImageByNumber(data) {
    ...
    async getArrivalImage(orderId, userId) {
        if (!orderId)
            return { success: false, message: '订单ID不能为空' };
        if (!userId)
            return { success: false, message: '用户ID不能为空' };
    ...
    async uploadVoiceFeedback(orderId, data) {
        if (!orderId)
            return { success: false, message: '订单ID不能为空' };
        if (!data.audioUrl)
            return { success: false, message: '音频URL不能为空' };
        if (!data.userId)
            return { success: false, message: '用户ID不能为空' };
        const order = await this.prisma.order.findUnique({ where: { id: orderId } });
        if (!order)
