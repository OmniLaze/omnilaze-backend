...
export class AuthService {
   ...
   * 尝试通过SPUG_URL发送验证码
   */
  private async sendViaSPUG(phoneNumber: string): Promise<{ success: boolean; message: string; data?: any; code?: string }> {
   ...
   * 尝试通过阿里云发送验证码（备用方案）
   */
  private async sendViaAliyun(phoneNumber: string): Promise<{ success: boolean; message: string; data?: any; code?: string }> {
    const accessKeyId = process.env.ALIYUN_ACCESS_KEY_ID;
    const accessKeySecret = process.env.ALIYUN_ACCESS_KEY_SECRET;
